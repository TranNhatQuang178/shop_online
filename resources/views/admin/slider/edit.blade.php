...
@section('content')
    <div class="content-wrapper">
        ...
        <section class="content">
            <form action="" id="form-update-slider">
                @csrf
                <!-- Default box -->
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <textarea name="desc" id="desc" cols="30" rows="7" class="form-control">{{ $slider->desc }}</textarea>
                                        <p></p>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <img src="{{ asset('/images/sliders/'. $slider->image) }}" alt="" width="100%" height="auto">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select name="status" id="" class="form-control">
                                            <option value="0" {{ $slider->status == 0 ? 'selected' : '' }}>Hide
                                            </option>
                                            <option value="1" {{ $slider->status == 1 ? 'selected' : '' }}>Display
                                            </option>
                                        </select>
                                    </div>
                                </div>
...
@section('js')
    <script>
        ...
        $("#form-update-slider").submit(function(e) {
            e.preventDefault();
            var element = $(this);
            $.ajax({
                headers: {
                ...
                success: function(response) {
                    if (response.errors) {
                        ...
                        if (error.image_id) {
                            $("#image_id").addClass('is-invalid').siblings('p').addClass(
                                    'invalid-feedback')
                                .text(error.image_id)
                        } else {
